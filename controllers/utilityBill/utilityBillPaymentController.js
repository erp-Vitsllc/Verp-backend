...
function decorateBill(bill) {
    ...
    o.inZoho = utilityBillHasZohoLink(o);
    return o;
}
 ...
 * Accounts + difference: HR approve goes to Zoho (no second Accounts approve).
...
function resolveStageAfterActor({
    ...
    const accountsDone = accountsAlreadyDone === true || actorIsAccounts;
    ...
    // hr_approve: Accounts-submitted difference → Zoho. Otherwise Accounts opens Zoho.
    if (accountsDone) {
        return { status: 'Approved', pendingRole: 'accounts', skipped: ['accounts'] };
    }
    return { status: 'Pending Accounts', pendingRole: 'accounts', skipped: [] };
}

function billFirstPendingRole(bill = {}) {
    const stored = String(bill.firstPendingRole || '').trim();
    if (stored === 'hr' || stored === 'accounts') return stored;
    if (String(bill.status || '') === 'Pending HR') return 'hr';
    if (String(bill.status || '') === 'Pending Accounts' && !bill.hrApprovedBy) {
        return 'accounts';
    }
    return '';
}

function actorOwnsBill(actor, bill) {
    return isSameEmployee(actor, bill?.requestedBy);
}

/** Creator or admin may change a bill until it exists in Zoho Books. */
function actorCanMutateUtilityBill(actor, bill, { isAdmin = false } = {}) {
    if (!bill) return false;
    if (utilityBillHasZohoLink(bill)) return false;
    if (String(bill.status || '') === 'Paid') return false;
    if (isAdmin === true) return true;
    if (!actor) return false;
    return actorOwnsBill(actor, bill);
}

function actorCanCreatorResend(actor, bill, canApproveReject = false, { isAdmin = false } = {}) {
    if (canApproveReject) return false;
    if (!actorCanMutateUtilityBill(actor, bill, { isAdmin })) return false;
...
function clearUtilityBillZohoFields(bill) {
    if (!bill) return;
    bill.zohoBillId = '';
    bill.zohoBillIds = [];
    bill.zohoBillStatus = '';
...
function applySubmitStageToBills(bills, { stage, actor, accounts, hr, needsHr, now }) {
    ...
    for (const bill of bills) {
        clearUtilityBillZohoFields(bill);
        bill.status = stage.status;
        bill.pendingWithName = pendingWithName;
        bill.pendingWithRole = pendingWithRole;
        bill.firstPendingRole =
            stage.status === 'Pending HR'
                ? 'hr'
                : stage.status === 'Pending Accounts'
                  ? 'accounts'
                  : '';
        ...
        bill.actionedAt = now;
        bill.comment = 'Resubmitted after rejection — sent through approval again';
        if (stage.skipped.includes('accounts')) {
            bill.accountsApprovedBy = actor?._id || null;
            bill.accountsApprovedAt = now;
            bill.comment = needsHr
                ? 'Accounts submitted difference — waiting HR; Zoho after HR approve'
                : 'Accounts step skipped (submitter is Accounts)';
        }
        if (stage.skipped.includes('hr')) {
            bill.hrApprovedBy = actor?._id || null;
            bill.hrApprovedAt = now;
            bill.comment = stage.skipped.includes('accounts')
    ...
    return { pendingWithName, pendingWithRole };
}

async function notifyAfterSubmitStage({
    bills,
    stage,
    requester,
    requestedByName,
    ...
    const leanBills = bills.map((b) => (typeof b.toObject === 'function' ? b.toObject() : b));
    const first = leanBills[0];
    if (!first) return { path: '', leanBills };
    const batchId = first.batchId;
    const path = reviewPath(batchId, first.utilityType, first.billMonth, first);
...
/**
 * After an individual bill in a group is edited or deleted, keep the pending
 * inbox totals in sync without creating a new task (and without emailing).
 */
async function refreshUtilityBillBatchInbox(batchId, { comment = '' } = {}) {
    const id = String(batchId || '').trim();
    if (!id) return { remainingPending: 0 };

    const remaining = await UtilityBillPayment.find({
        batchId: id,
    ...
    if (!remaining.length) {
        await DashboardAction.updateMany(
            { requestType: REQUEST_TYPE, requestId: id, status: 'Pending' },
            {
                status: 'Approved',
                actionedDate: new Date(),
                comment: comment || 'No remaining pending bills in this group',
            },
        );
    ...
    const total = remaining.reduce((sum, bill) => sum + (Number(bill.amount) || 0), 0);
    const utilityType = first?.utilityType || 'Utility';
    const billMonth = first?.billMonth || '';
    const path = reviewPath(id, utilityType, billMonth, first);
    await DashboardAction.updateMany(
        { requestType: REQUEST_TYPE, requestId: id, status: 'Pending' },
        {
            $set: {
                extra1: `${utilityType} ${billMonth || ''} — ${remaining.length} bill(s) · ${total.toLocaleString()} AED`.trim(),
                extra2: statusLabel(first?.status, first?.pendingWithName),
                extra3: JSON.stringify({
                    batchId: id,
                    entryId: String(first?.entryId || ''),
                    billId: String(first?._id || ''),
    ...
    return { remainingPending: remaining.length };
}
    ...
    try {
        ...
        const focus =
            ...
            bills.find(
                (b) =>
                    b.status === 'Rejected' &&
                    actorCanCreatorResend(actor, b, false, { isAdmin }),
            ) ||
            bills[0];
 ...
 * Accounts + difference → HR, then Zoho (no second Accounts approve).
 * HR approve (user bill) → Accounts creates Zoho. HR reject → Rejected.
...
export async function updateUtilityBillBatch(req, res) {
    try {
        ...
        const hrGate = await isActorHrOrAdmin(actor, req.user);
        const isAdmin = isUtilityAdminSuperUser(req);
        let existingBills = await UtilityBillPayment.find({
            batchId,
            status: { $in: ['Pending Accounts', 'Pending HR', 'Approved', 'Rejected'] },
        });
        if (!existingBills.length) {
            const single = await UtilityBillPayment.findById(batchId);
            if (single?.batchId) {
                existingBills = await UtilityBillPayment.find({
                    batchId: single.batchId,
                    status: { $in: ['Pending Accounts', 'Pending HR', 'Approved', 'Rejected'] },
                });
            } else if (
                single &&
                ['Pending Accounts', 'Pending HR', 'Approved', 'Rejected'].includes(
                    String(single.status),
                )
            ) {
                existingBills = [single];
            }
        }
        if (!existingBills.length) {
        ...
        const requestedIds = rows
            .map((row) => String(row.billId || row._id || '').trim())
            .filter(Boolean);
        const requestedBills = requestedIds.length
            ? existingBills.filter((bill) => requestedIds.includes(String(bill._id)))
            : [];
        if (
            requestedBills.some((bill) => String(bill.status) === 'Rejected') &&
            requestedBills.some((bill) => String(bill.status) !== 'Rejected')
        ) {
            return res.status(400).json({
                message: 'Rejected bills must be edited and sent again on their own.',
            });
        }

        const rejectedTargets = existingBills.filter(
            (bill) =>
                String(bill.status) === 'Rejected' &&
                actorCanCreatorResend(actor, bill, false, { isAdmin }) &&
                (!requestedIds.length || requestedIds.includes(String(bill._id))),
        );
        const resubmitRejected =
            rejectedTargets.length > 0 &&
            (requestedBills.length
                ? requestedBills.every((bill) => String(bill.status) === 'Rejected')
                : existingBills.every((bill) => String(bill.status) === 'Rejected'));
        ...
        if (resubmitRejected) {
            ...
            const remoteDuplicates = await lookupVendorBillDuplicates(rows, {
                excludeIds: rejectedTargets.map((bill) => bill._id),
            });
            const remoteHit = remoteDuplicates.find((row) => row.source);
            if (remoteHit) {
                return res.status(400).json({ message: remoteHit.message });
            ...
            applyRowEdits(rejectedTargets, rows);

            const leftoverLive = existingBills.filter(
                (bill) =>
                    !rejectedTargets.some((target) => String(target._id) === String(bill._id)) &&
                    ['Pending Accounts', 'Pending HR', 'Approved'].includes(String(bill.status)),
            );
            if (leftoverLive.length) {
                const newBatchId = new mongoose.Types.ObjectId();
                for (const bill of rejectedTargets) {
                    bill.batchId = newBatchId;
                }
            }

            const originalRequesterId = rejectedTargets[0]?.requestedBy;
            const submitter =
                originalRequesterId && isSameEmployee(actor, originalRequesterId)
                    ? actor
                    : originalRequesterId
                      ? await EmployeeBasic.findById(originalRequesterId)
                            .select(
                                'firstName lastName companyEmail workEmail personalEmail email employeeId',
                            )
                            .lean()
                      : actor;
            const needsHr = batchNeedsHrApproval(rejectedTargets);
            const stage = resolveStageAfterActor({
                actor: submitter || actor,
                accounts,
                hr,
                from: 'submit',
                needsHr,
            ...
            const now = new Date();
            applySubmitStageToBills(rejectedTargets, {
                stage,
                actor,
                accounts,
                hr,
            ...
            let zohoSync = null;
            if (stage.status === 'Approved') {
                const splitCheck = applyPaySplitToBills(
                    rejectedTargets,
                    rejectedTargets[0]?.paymentBy,
                );
                if (!splitCheck.ok) {
                    return res.status(400).json({ message: splitCheck.message });
                }
            ...
            if (stage.status === 'Approved') {
                zohoSync = await syncApprovedUtilityBillsToZoho(rejectedTargets, {
                    markAsOpen: true,
                });
                try {
                    await upsertUtilityBalancePartyExpensesFromBills(
                    ...
                    console.warn(
                        '[updateUtilityBillBatch] PartyExpense balance upsert failed:',
                        balanceErr?.message || balanceErr,
                    );
                }
                notifyUtilityBillZohoPayableParties({
                    bills: rejectedTargets,
                    batchMeta: {
                        batchId: String(rejectedTargets[0]?.batchId || batchId),
                        billCount: rejectedTargets.length,
                        reviewPath: reviewPath(
                            rejectedTargets[0]?.batchId || batchId,
                            rejectedTargets[0]?.utilityType,
                            rejectedTargets[0]?.billMonth,
                            rejectedTargets[0],
                        ),
                    },
                }).catch((err) =>
                    console.warn(
                        '[updateUtilityBillBatch] Zoho payable notify failed:',
                        err?.message || err,
                    ),
                );
            }
            ...
            const requesterDoc =
                originalRequesterId && !isSameEmployee(actor, originalRequesterId)
                    ? submitter
                    : actor;
            await notifyAfterSubmitStage({
                bills: rejectedTargets,
                stage,
                requester: requesterDoc,
                requestedByName: rejectedTargets[0]?.requestedByName || '',
                accounts,
                hr,
            });

            const outBatchId = String(rejectedTargets[0]?.batchId || batchId);
            const refreshed = await UtilityBillPayment.find({
                _id: { $in: rejectedTargets.map((bill) => bill._id) },
            }).lean();
            return res.status(200).json({
                batchId: outBatchId,
                updatedCount: rejectedTargets.length,
                ...
                skippedStages: stage.skipped,
                zohoSync,
                message: 'Edited and sent again through the approval flow.',
            });
        }

        const liveBills = existingBills.filter((bill) => String(bill.status) !== 'Rejected');
        if (!liveBills.length) {
            return res.status(403).json({
                message:
                    'This rejected bill can be edited and sent again by the person who submitted it.',
            });
        }
        ...
        const canEditAccounts = stageStatus === 'Pending Accounts' && accountsGate.allowed;
        const canEditHr = stageStatus === 'Pending HR' && hrGate.allowed;
        const canEditApproved = stageStatus === 'Approved' && accountsGate.allowed;
        const targetPool = requestedIds.length
            ? liveBills.filter((bill) => requestedIds.includes(String(bill._id)))
            : liveBills;
        const permissionFocus = targetPool[0] || liveBills[0];
        const canMutateTargets =
            targetPool.length > 0 &&
            targetPool.every((bill) => actorCanMutateUtilityBill(actor, bill, { isAdmin }));
        const canCreatorResend = actorCanCreatorResend(
            actor,
            permissionFocus,
            canEditAccounts || canEditHr,
            { isAdmin },
        );
        if (
            !canEditAccounts &&
            ...
            return res.status(403).json({
                message:
                    permissionFocus && actorOwnsBill(actor, permissionFocus)
                        ? 'This bill cannot be edited after it is added in Zoho Books.'
                        : 'Only the person who submitted this bill (before Zoho) or the pending Accounts / HR approver can edit it.',
            });
        }

        const creatorOnly =
            (canCreatorResend || canMutateTargets) &&
            !canEditAccounts &&
            !canEditHr &&
            !canEditApproved;
        let bills = creatorOnly ? targetPool : liveBills;
        if (requestedIds.length) {
            bills = bills.filter((bill) => requestedIds.includes(String(bill._id)));
        }
        if (!bills.length) {
            return res.status(404).json({
                message: 'No editable bills found for this batch (Paid bills cannot be edited).',
        ...
        if (creatorOnly && bills.some((bill) => utilityBillHasZohoLink(bill))) {
            return res.status(400).json({
                message: 'This bill is already in Zoho Books and cannot be edited.',
            });
        }
        ...
        const creatorResend = Boolean(creatorOnly);
        for (const bill of bills) {
            clearUtilityBillZohoFields(bill);
            if (creatorResend) {
                bill.comment = 'Edited by creator — awaiting current approver (no email sent)';
                bill.actionedBy = actor?._id || null;
        ...
        if (creatorResend) {
            const inboxBatchId = String(bills[0]?.batchId || batchId || '');
            await refreshUtilityBillBatchInbox(inboxBatchId, {
                comment: 'Creator edited a bill in this group',
            });
        }
        ...
        return res.status(200).json({
            ...
            bills: refreshed.map((b) => decorateBill(b)),
            resent: creatorResend,
            message: creatorResend
                ? 'Bill updated. Accounts is not emailed for edits.'
                : 'Bill details saved. Retry Zoho sync or Pay to store the updated bill.',
        });
    } catch (err) {
...
async function resolveSelectedBillsForRespond({
    ...
    for (const row of selectedNew) {
        ...
        const doc = {
            ...
            firstPendingRole: template.firstPendingRole || billFirstPendingRole(template),
        };
        if (
            payBy === 'company' ||
            payBy === 'employee' ||
...
export async function respondUtilityBillBatch(req, res) {
    try {
        ...
        const accountsAlreadyDone =
            bills.every((b) => Boolean(b.accountsApprovedBy)) ||
            isSameEmployee(requester, accounts);
        const nextAfterHr = resolveStageAfterActor({
            actor,
            accounts,
            hr: gate.hr,
            from: 'hr_approve',
        ...
        if (nextAfterHr.status === 'Approved') {
            const splitCheckHrZoho = applyPaySplitToBills(bills, paymentBy);
            if (!splitCheckHrZoho.ok) {
                return res.status(400).json({ message: splitCheckHrZoho.message });
            }

            const zohoSyncHr = await syncApprovedUtilityBillsToZoho(bills, { markAsOpen: true });
            await Promise.all(bills.map((b) => b.save()));
            const zohoFailedHr = (zohoSyncHr || []).filter(
                (r) => r && r.ok === false && !r.skipped,
            );
            const stillDraftHr = bills.filter(
                (b) => String(b.zohoBillStatus || '').toLowerCase() !== 'open',
            );
            if (zohoFailedHr.length || stillDraftHr.length) {
                const firstMsg =
                    zohoFailedHr[0]?.message ||
                    stillDraftHr[0]?.zohoSyncError ||
                    'Zoho bill is still Draft. Open it in Zoho before payment.';
                return res.status(400).json({
                    batchId,
                    status: 'Pending HR',
            ...
            for (const bill of bills) {
                ...
                if (!bill.accountsApprovedBy) {
                    bill.accountsApprovedBy = actor?._id || null;
                    bill.accountsApprovedAt = nowHr;
                }
                bill.actionedBy = actor?._id || null;
                bill.actionedAt = nowHr;
                bill.comment =
                    comment ||
                    'Approved by HR — Accounts already submitted — Zoho Open — awaiting Pay';
                await bill.save();
            }
            ...
            const remainingHrAfterZoho = await UtilityBillPayment.countDocuments({
                batchId,
                status: 'Pending HR',
            });
            const hrApprovalFinished = remainingHrAfterZoho === 0;
            await syncBatchDashboard({
                batchId,
                bills: bills.map((b) => b.toObject()),
                assignedTo: accounts._id,
                status: hrApprovalFinished ? 'Approved' : 'Pending',
                actionedBy: actor?._id || req.user?._id,
                comment:
                    comment ||
                    'Approved by HR — Zoho Open — awaiting Accounts payment',
                subjectEmployee: requester,
                requestedByName: allBills[0].requestedByName,
                extra2: hrApprovalFinished
                    ? 'not paid — awaiting Accounts payment'
                    : statusLabel('Pending HR', empDisplayName(gate.hr)),
            });

            if (hrApprovalFinished) {
                await sendUtilityBillPaymentEmail({
                    recipient: accounts,
                    bill: {
                        ...bills[0].toObject(),
            ...
            return res.status(200).json({
                ...
                status: 'Approved',
                statusLabel: 'not paid',
                skippedStages: nextAfterHr.skipped,
                bills: bills.map((b) => decorateBill(b.toObject())),
                zohoSync: zohoSyncHr,
            });
        }
...
export async function deleteUtilityBillPayment(req, res) {
    try {
        ...
        const status = String(bill.status || '');
        const wasSentForApproval = ['Pending Accounts', 'Pending HR', 'Approved', 'Rejected'].includes(
            status,
        );
        const batchId = bill.batchId ? String(bill.batchId) : '';
        const deletedByName =
            empDisplayName(actor) || requesterDisplayName(actor, req.user) || 'A user';
        ...
        if (batchId) {
            try {
                await refreshUtilityBillBatchInbox(batchId, {
                    comment: `${deletedByName} deleted a bill from this group`,
                });
            } catch (inboxErr) {
                console.warn(
                    '[deleteUtilityBillPayment] Inbox refresh failed:',
        ...
        if (wasSentForApproval) {
            try {
                const accounts = await getDepartmentHOD('accounts');
                if (accounts?._id) {
                    const remainingPending = batchId
                        ? await UtilityBillPayment.countDocuments({
                              batchId,
                              status: { $in: ['Pending Accounts', 'Pending HR'] },
                          })
                        : 0;
                    await sendUtilityBillPaymentEmail({
                        ...
                        batchMeta: {
                            batchId,
                            billCount: 1,
                            reviewPath: billDetailsPath({
                                entryId: snapshot.entryId,
                            }),
                            comment:
                                `${deletedByName} deleted this bill` +
                                (remainingPending > 0
                                    ? `. ${remainingPending} bill(s) remain in the group.`
                                    : '. No pending bills remain in this group.'),
                        },
                    });
                }
            } catch (mailErr) {
                console.warn(
                    '[deleteUtilityBillPayment] Delete email failed:',
        ...
        return res.json({
            ...result,
            emailedAccounts: wasSentForApproval,
        });
    } catch (err) {

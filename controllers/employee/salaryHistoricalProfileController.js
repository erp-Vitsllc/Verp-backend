...
function emptyLiveAttendance(from = '', to = '', workingDays = 0) {
    return {
        workingDays,
        leaveRecords: [],
        overtimeHours: 0,
    ...
    return rows.slice(0, 40).map((row, index) => {
        ...
        return {
            ...
            allocations: Array.isArray(row?.allocations)
                ? row.allocations
                      .map((item) => ({
                          kind: String(item?.kind || 'leave').trim() === 'ticket' ? 'ticket' : 'leave',
                          entitlementNo: Number(item?.entitlementNo) || 0,
                          entitlementDate: toDateKey(item?.entitlementDate),
...
export async function buildPayload(req, employeeId, overlay = {}) {
    ...
    return {
        ...
        liveAttendance: {
            ...
            overtimeHours: Number(liveAttendance.overtimeHours) || 0,
            overtimeDays: Number(liveAttendance.overtimeDays) || 0,
            overtimeRecords: liveAttendance.overtimeRecords || [],
...
async function upsertFromBody(req, employeeId, extra = {}) {
    ...
    const paidLeaveError = paidLeaveSalaryMutationError(existing?.paymentCycles, paymentCycles);
    if (paidLeaveError) {
        const err = new Error(paidLeaveError);
        err.statusCode = 400;
        throw err;
    }

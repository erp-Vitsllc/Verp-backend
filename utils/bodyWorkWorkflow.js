...
export function isBodyWorkServiceType(serviceType) {
    return String(serviceType || '').trim() === 'Body Work';
}

export function isBodyWorkWorkflow(wf, serviceSub) {
    return isBodyWorkServiceType(wf?.serviceTypeLabel || serviceSub?.serviceType);
}

export function bodyWorkDetailsPath(vehicleId, serviceRecordId) {
    if (!vehicleId || !serviceRecordId) return null;
    return `/HRM/Asset/Vehicle/details/${vehicleId}/body-work/${serviceRecordId}`;
}

export function bodyWorkDashboardMeta(asset, serviceRecordId) {
    const path = bodyWorkDetailsPath(asset?._id, serviceRecordId);
...
/** Persist body-work timeline rows on the service remark (draft steps before workflow history). */
export function appendBodyWorkActivity(service, { type, byName, note = '' }) {
...
export async function notifyBodyWorkStakeholder({
    asset,
    serviceRecordId,
    recipient,
    extra2,
    ...
    actionLabel = 'Body work workflow',
...
export async function resolveBodyWorkAssigneeForStage(stage) {
    const s = String(stage || '').toLowerCase();
    if (s === BODY_WORK_STAGE.HR) return getDepartmentHOD('hr');
    if (s === BODY_WORK_STAGE.ACCOUNTS) return getDepartmentHOD('accounts');
    if (s === BODY_WORK_STAGE.ADMIN_OFFICER || s === BODY_WORK_STAGE.ADMIN_RETURN) {
...
export async function advanceBodyWorkAfterAccountsApprove(asset, wf, actorName) {
    ...
    return advanceShopServiceToScheduledAfterAccountsApprove(asset, wf, actorName, {
        ...
        linkPath: bodyWorkDetailsPath(asset._id, wf.serviceRecordId),
        dashboardMeta: bodyWorkDashboardMeta(asset, wf.serviceRecordId),
        appendActivity: appendBodyWorkActivity,
    });
}

async function actorMayEditBodyWorkQuoteEmployeeRows(reqUser, asset) {
...
export async function updateBodyWorkQuoteEmployeeRows(asset, serviceId, employeeRows, req) {
    const service = asset.services?.id?.(serviceId);
    if (!service) throw new Error('Service record not found');
    if (!isBodyWorkServiceType(service.serviceType)) {
        throw new Error('Not a mechanical work service record.');
    }

    const wf = asset.activeServiceWorkflow || {};
    if (!isBodyWorkWorkflow(wf, service)) {
    ...
    if ([BODY_WORK_STAGE.COMPLETE, BODY_WORK_STAGE.REJECTED].includes(stage)) {
        throw new Error('Employee rows cannot be edited after the workflow is finished.');
    }

    const allowed = await actorMayEditBodyWorkQuoteEmployeeRows(req.user, asset);
...
export async function submitBodyWorkGarage(asset, serviceId, serviceUpdates, req) {
    const service = asset.services?.id?.(serviceId);
    if (!service) throw new Error('Service record not found');
    const wf = asset.activeServiceWorkflow || {};
    if (!isBodyWorkWorkflow(wf, service)) throw new Error('Not a body work workflow.');
    if (String(wf.serviceRecordId) !== String(serviceId)) {
        throw new Error('Service record does not match active workflow.');
    }
    const stage = String(wf.stage || '').toLowerCase();
...
export async function createBodyWorkEmployeeFines(asset, service, reqUser) {
    const remark = parseRemark(service);
    ...
    remark.bodyWorkFinesCreatedAt = new Date().toISOString();
    service.remark = JSON.stringify(remark);
    asset.markModified('services');
...
export async function completeBodyWorkService(asset, serviceId, serviceUpdates, req) {
    const service = asset.services?.id?.(serviceId);
    if (!service) throw new Error('Service record not found');
    const wf = asset.activeServiceWorkflow || {};
    if (!isBodyWorkWorkflow(wf, service)) throw new Error('Not a body work workflow.');
    ...
    remark.workflowStage = BODY_WORK_STAGE.COMPLETE;
    remark.serviceCompletedByName = actorName;
    asset.services.id(serviceId).remark = JSON.stringify(remark);
    appendBodyWorkActivity(asset.services.id(serviceId), {
        type: 'service_completed',
        byName: actorName,
        note: 'Body work service completed',
    });

    wf.stage = BODY_WORK_STAGE.COMPLETE;
...
export async function notifyBodyWorkAccountsHoldToAdmin(asset, wf, holdReason, actorName) {
    const adminOfficer = await getDepartmentHOD('admincontroller');

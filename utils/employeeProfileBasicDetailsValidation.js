...
function validateProfileEmail(value) {
    ...
    if (email.length > 100) return "Email must be no more than 100 characters";
...
function validateProfileDateOfBirth(value) {
    ...
    if (startOfDay(d) > startOfDay(new Date())) return "Date of Birth cannot be in the future";
...
function validateProfileFathersName(value) {
    ...
    if (!PROFILE_FATHER_NAME.test(cleaned)) {
        return "Father's Name must contain only letters, spaces, apostrophe and hyphen";
    }
    return null;
}

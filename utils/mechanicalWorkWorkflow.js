...
export const MECHANICAL_WORK_STAGE = {
    HR: 'pending_hr',
...
export function isMechanicalWorkServiceType(serviceType) {
    return String(serviceType || '').trim() === 'Mechanical Work';
}

export function isMechanicalWorkWorkflow(wf, serviceSub) {
    return isMechanicalWorkServiceType(wf?.serviceTypeLabel || serviceSub?.serviceType);
}

export function mechanicalWorkDetailsPath(vehicleId, serviceRecordId) {
    if (!vehicleId || !serviceRecordId) return null;
    return `/HRM/Asset/Vehicle/details/${vehicleId}/mechanical-work/${serviceRecordId}`;
}

export function mechanicalWorkDashboardMeta(asset, serviceRecordId) {
    const path = mechanicalWorkDetailsPath(asset?._id, serviceRecordId);
...
/** Persist mechanical-work timeline rows on the service remark (draft steps before workflow history). */
export function appendMechanicalWorkActivity(service, { type, byName, note = '' }) {
...
export async function notifyMechanicalWorkStakeholder({
    asset,
    serviceRecordId,
    recipient,
    extra2,
    ...
    actionLabel = 'Mechanical work workflow',
    stageLabel = 'Action required',
}) {
    if (!recipient?._id) return;
    const linkPath = mechanicalWorkDetailsPath(asset._id, serviceRecordId);
...
export async function resolveMechanicalWorkAssigneeForStage(stage) {
    const s = String(stage || '').toLowerCase();
    if (s === MECHANICAL_WORK_STAGE.HR) return getDepartmentHOD('hr');
    if (s === MECHANICAL_WORK_STAGE.ACCOUNTS) return getDepartmentHOD('accounts');
    if (s === MECHANICAL_WORK_STAGE.ADMIN_OFFICER || s === MECHANICAL_WORK_STAGE.ADMIN_RETURN) {
        return getDepartmentHOD('admincontroller');
    }
    return null;
}

export async function advanceMechanicalWorkAfterHrApprove(asset, wf, actorName) {
    ...
    await routeShopServiceToAccountsApproveAfterGarage(asset, serviceRecordId, {
        ...
        actorName,
        linkPath: mechanicalWorkDetailsPath(asset._id, serviceRecordId),
        dashboardMeta: mechanicalWorkDashboardMeta(asset, serviceRecordId),
        appendActivity: null,
        openedBy: 'hr',
    });
}

export async function advanceMechanicalWorkAfterAccountsApprove(asset, wf, actorName) {
    ...
    return advanceShopServiceToScheduledAfterAccountsApprove(asset, wf, actorName, {
        serviceTypeLabel: 'Mechanical Work',
        linkPath: mechanicalWorkDetailsPath(asset._id, wf.serviceRecordId),
        dashboardMeta: mechanicalWorkDashboardMeta(asset, wf.serviceRecordId),
        appendActivity: appendMechanicalWorkActivity,
    });
}

async function actorMayEditMechanicalWorkQuoteEmployeeRows(reqUser, asset) {
...
export async function updateMechanicalWorkQuoteEmployeeRows(asset, serviceId, body, req) {
    const service = asset.services?.id?.(serviceId);
    if (!service) throw new Error('Service record not found');
    if (!isMechanicalWorkServiceType(service.serviceType)) {
        throw new Error('Not a mechanical work service record.');
    }

    const { wf, bindActive } = getWorkflowContextForService(asset, serviceId);
    if (!wf || !isMechanicalWorkWorkflow(wf, service)) {
        throw new Error('No active mechanical work workflow for this service.');
    }
    const stage = String(wf.stage || '').toLowerCase();
    if ([MECHANICAL_WORK_STAGE.COMPLETE, MECHANICAL_WORK_STAGE.REJECTED].includes(stage)) {
        throw new Error('Employee rows cannot be edited after the workflow is finished.');
    }

    const allowed = await actorMayEditMechanicalWorkQuoteEmployeeRows(req.user, asset);
...
export async function submitMechanicalWorkGarage(asset, serviceId, serviceUpdates, req) {
    const service = asset.services?.id?.(serviceId);
    if (!service) throw new Error('Service record not found');
    const { wf, bindActive } = getWorkflowContextForService(asset, serviceId);
    if (!wf || !isMechanicalWorkWorkflow(wf, service)) throw new Error('Not a mechanical work workflow.');
    const stage = String(wf.stage || '').toLowerCase();
    ...
    const mayUpdateGarage =
        stage === MECHANICAL_WORK_STAGE.HR ||
        stage === MECHANICAL_WORK_STAGE.ADMIN_OFFICER ||
        (stage === MECHANICAL_WORK_STAGE.ACCOUNTS && !garageAlreadySubmitted);
    if (!mayUpdateGarage) {
    ...
    if (stage === MECHANICAL_WORK_STAGE.HR) {
        const { snapshotActiveServiceWorkflow } = await import('./vehicleServiceWorkflowResolve.js');
        if (!bindActive) {
            snapshotActiveServiceWorkflow(asset);
        }
    ...
    if (accountsAlreadyDone || stage === MECHANICAL_WORK_STAGE.ACCOUNTS) {
        const { snapshotActiveServiceWorkflow } = await import('./vehicleServiceWorkflowResolve.js');
        if (!bindActive) {
            snapshotActiveServiceWorkflow(asset);
        }
        asset.activeServiceWorkflow = {
            ...(typeof wf.toObject === 'function' ? wf.toObject() : wf),
            stage: accountsAlreadyDone ? stage : MECHANICAL_WORK_STAGE.ACCOUNTS,
            serviceRecordId: wf.serviceRecordId || serviceId,
            serviceTypeLabel: wf.serviceTypeLabel || 'Mechanical Work',
            history: Array.isArray(wf.history) ? [...wf.history] : [],
            garageSubmittedAt: wf.garageSubmittedAt,
        ...
        if (accountsAlreadyDone) {
            ...
            if (accounts?._id) {
                await notifyMechanicalWorkStakeholder({
                    ...
                    actionLabel: 'Mechanical work — Schedule updated',
                    detailLine: `${actorName} completed Schedule/garage for mechanical work. Review and approve so Ready / On Service can start.`,
                });
            }
        }
        return asset;
...
export async function createMechanicalWorkEmployeeFines(asset, service, reqUser) {
    const remark = parseRemark(service);
    ...
    remark.mechanicalWorkFinesCreatedAt = new Date().toISOString();
    service.remark = JSON.stringify(remark);
    asset.markModified('services');
...
export async function completeMechanicalWorkService(asset, serviceId, serviceUpdates, req) {
    const service = asset.services?.id?.(serviceId);
    if (!service) throw new Error('Service record not found');
    const { wf, bindActive } = getWorkflowContextForService(asset, serviceId);
    if (!wf || !isMechanicalWorkWorkflow(wf, service)) throw new Error('Not a mechanical work workflow.');
    ...
    const mayComplete =
        stage === MECHANICAL_WORK_STAGE.ADMIN_RETURN ||
        stage === 'pending_admin' ||
    ...
    if (stage === 'pending_admin') {
        wf.stage = MECHANICAL_WORK_STAGE.ADMIN_RETURN;
    }
...
export async function notifyMechanicalWorkAccountsHoldToAdmin(asset, wf, holdReason, actorName) {
